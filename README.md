...
It largely follows conventional commits (see [Extra Links](#links)),
but has been extended to prevent most commits from falling into either `feat` or `chore`.
...
Temporary commits that should be squashed later on can be marked simply as
`WIP` (for work-in-progress) or `FIXUP`.
